...
my $db = $opt_m;
my $species = $opt_o;
my ($dbname, $dbhost, $dbport, $dbuser, $dbpass) = get_db($db);
...
while ($sth->fetchrow_arrayref()) {
	...
	# @mirnas = [$mirna, $accession, $coord, $seq, $mature_coord (3p), $mature_acc, $mature_coord (5p), '', '', '']
		...
		if ($mature !~ /\*/) {
			...
			if ($mature_coord ne $mirnas[-1][4]) {
				if ($mirnas[-1][6] eq '') {
					...
					print STDERR "miRNA $mirna has more than 2 mature coords: $mirnas[-1][4], $mirnas[-1][5], $mature_coord\n";
					if ($flag == 0) {
						push (@mirnas, [$mirna, $accession, $coord, $seq, $mature_coord, $mature_acc, '', '', '', '']);
						$flag = 1;
					}elsif($flag == 1){
