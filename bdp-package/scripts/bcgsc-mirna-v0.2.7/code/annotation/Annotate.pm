...
sub mirbase_hashes {
	...
	my ($dbname, $dbhost, $dbport, $dbuser, $dbpass) = get_db($mirbase);
	my $dbh_mirbase = DBI->connect("DBI:mysql:database=$dbname;host=$dbhost;port=$dbport", $dbuser, $dbpass, {AutoCommit => 0, PrintError => 1}) || die "Could not connect to database: $DBI::errstr";
...
sub ucsc_hashes {
	...
	my ($dbname, $dbhost, $dbport, $dbuser, $dbpass) = get_db($ucsc_db);
	my $dbh_ucsc = DBI->connect("DBI:mysql:database=$dbname;host=$dbhost;port=$dbport", $dbuser, $dbpass, {AutoCommit => 1, PrintError => 1}) || die "Could not connect to database: $DBI::errstr"; #autocommit set to try to allow auto_reconnect
...
#	my $db_connections = "$dir/../../config/db_connections.cfg";
	my $db_connections = "/package/scripts/bcgsc-mirna-v0.2.7/config/db_connections.cfg";
